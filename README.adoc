...
*Le code est-il structuré en couches ? Quel problèmes ce code peut-il poser ?*
 Réponse : Le code est bien structuré car nous avons trouvé : 
        - Un repository pour intéragir avec la base de données
        - Un controlleur
        - Ressource pour acceder à la base de données
        - Le code metier
*Où se trouve le code métier (voir la règle de gestion RG 1) ?*
        - Le code metier se trouve dans le fichier TodoListController.java
*Cette règle est-elle facilement testable par un test unitaire ?*
        - Oui facilement par un test unitaire car elle est contenu dans TodoListController qui est dans une méthode isolée.   
...
*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?*
 Parce qu'il est pas bien injecté. Pour l'injecter il faut utiliser @inject juste avant le constructeur
